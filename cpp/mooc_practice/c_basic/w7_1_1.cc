...
int main()
{
    // 输入：每行是用空格分开的两个整数，第一个是n, 第二个是m (0 < m,n <=300)。最后一行是：
    int n, m;
    while (true)
    {
        cout << "输入猴子的数量n和数字m。" << endl;
        cin >> n >> m;
        // 输入"0 0"退出
        if (n == 0 && m == 0) 
        {
        ...
        while (true)
        {
            ...
            if (pre == suc)
            {
                ...
                // current是退出的，那么另一个（pre[current] | suc[current]）就是剩下的。
                // 我们的序号是从0编号的，输出时要加一。
                cout << "最后剩下的猴王是：" << pre + 1 << endl; // 这里用pre和suc都一样。
                break;
            }
            current = suc;
        }
