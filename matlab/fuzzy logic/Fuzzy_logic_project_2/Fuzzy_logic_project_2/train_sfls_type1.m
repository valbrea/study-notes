...
%% membership functions are Gaussian, using some input衞utput training
...
function [M,c0,sigma]=train_sfls_1(X,D,M,sigma,c0,alpha);   %这里应该就是这个测试？或者说是训练加测试？alpha应该是这个步长
...
for i=1:L
    ...
    fa=U/sum(U);
    f=fa*c0;
    fa=fa';   %前面都是训练过程？或者说是得到这个
    e=D(i)-f;   %表示这个误差？那么现在这个问题来了，就是如何找到这个height？
